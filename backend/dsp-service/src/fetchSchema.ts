...
export function schemaFromFile(schemaFile: string): DocumentNode {
  try {
    logger.debug('[fetchSchema.schemaFromFile] loading schemas from file: ', schemaFile);
    const typeDefsString = fs.readFileSync(schemaFile, 'utf8');
    logger.debug('[fetchSchema.schemaFromFile]compiling schema from file: ', schemaFile);
...
export function schemaFromDirectory(schemaDir: string): DocumentNode {
  try {
    logger.debug('[fetchSchema.schemaFromDirectory] loading schemas from files ', schemaDir);
    // const schemaFiles = fs.readdirSync(schemaDir);
    const schemaFiles = globSync(schemaDir + '/**/*.graphql');
    logger.debug('[fetchSchema.schemaFromDirectory] schema files: ', schemaFiles);
